...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!slug) throw new Error('No restaurant was selected. Open the website with /cafe-website/cafe or ?restaurant=cafe.');
  ...
  const addToCart = (product: Product) => setCart(current => {
    ...
    return found ? current.map(x => x.product.id === product.id ? { ...x, quantity: x.quantity + 1 } : x) : [...current, { product, quantity: 1 }];
  });
  ...
  return <div className="app" style={{ '--primary': branding.customer_primary_color || '#111', '--secondary': branding.customer_secondary_color || '#fff', '--background': branding.customer_background_color || '#f8f7f4' } as React.CSSProperties}>
    <header className="header">
      <div className="header-inner">
        <a className="brand" href={`?restaurant=${restaurant.slug}`}>
          {branding.customer_logo_url ? <img src={branding.customer_logo_url} alt={restaurant.name} /> : <span className="logo-fallback">{restaurant.name.charAt(0)}</span>}
          <span>{restaurant.name}</span>
        </a>
        <div className="header-actions">
          <a href="#about">About</a><a href="#locations">Locations</a>
          <button className="cart-button" onClick={() => document.getElementById('cart')?.scrollIntoView({ behavior: 'smooth' })}>Cart <b>{cartCount}</b></button>
        </div>
      </div>
    </header>

    <main>
      <section className="hero" style={branding.customer_cover_url ? { backgroundImage: `linear-gradient(90deg, rgba(0,0,0,.68), rgba(0,0,0,.2)), url(${branding.customer_cover_url})` } : undefined}>
        <div className="hero-content"><span className="eyebrow">Welcome</span><h1>{restaurant.name}</h1><p>{restaurant.description || 'Freshly prepared favorites, made for your day.'}</p><div className="hero-meta"><span>⏱ {settings.preparation_time_minutes} min prep</span>{settings.accepts_delivery && <span>Delivery available</span>}{settings.accepts_pickup && <span>Pickup available</span>}</div></div>
      </section>
      ...
      <section className="menu-section">
        <div className="section-heading"><div><span className="eyebrow">Our menu</span><h2>Choose your favorite</h2></div><input className="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search menu…" /></div>
        <div className="category-bar"><button className={activeCategory === 'all' ? 'active' : ''} onClick={() => setActiveCategory('all')}>All</button>{store.categories.map(c => <button key={c.id} className={activeCategory === c.id ? 'active' : ''} onClick={() => setActiveCategory(c.id)}>{c.name}</button>)}</div>
        <div className="product-grid">{products.map(product => <article className="product-card" key={product.id} onClick={() => setSelected(product)}>
          <div className="product-image">{product.image_url ? <img src={product.image_url} alt="" loading="lazy" /> : <span>{product.name.charAt(0)}</span>}{product.best_seller && <span className="badge">Best seller</span>}</div>
          <div className="product-body"><div><h3>{product.name}</h3><p>{product.description || 'Prepared fresh to order.'}</p></div><div className="product-footer"><strong>{money(Number(product.base_price), settings.currency)}</strong><button onClick={e => { e.stopPropagation(); addToCart(product); }}>+ Add</button></div></div>
        </article>)}</div>
